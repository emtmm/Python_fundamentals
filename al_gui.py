...
for f in EXPECTED_FNS:
    if not hasattr(al, f):
        print(f, "is missing in al.py.")
        al.__dict__[f] = lambda *x:0
...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.diff = al.hours_difference(dlt, 0)
    ...
    def refresh(self, forced=False):
        ...
        hour = int(al.time_from_utc(utc_hour, self.diff))
        ...
        minutes_angle = al.get_minutes(int(self.now)) / 60 * 360
        if al.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif al.time_from_utc(0, self.diff) % 1 == 0.75:
            minutes_angle += 270
        ...
        seconds_angle = int(al.get_seconds(int(self.now))) / 60 * 360
